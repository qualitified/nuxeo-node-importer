...
exports.import = function(config) {
  ...
  return nuxeo.batchUpload() // 🔥 RETURN ICI
    .upload(blob)
    .then(function(res) {
      ...
      return nuxeo.operation(config.chainId)
        ...
        .execute()
         .then(function(doc) {
           // Si doc est null, rejette la promesse
           if (!doc || doc['entity-type'] !== 'document') {
             return Promise.reject(new Error('Operation '+config.chainId+' return no document or null while processing file ' + file));
          }
          return doc; // sinon renvoie le doc pour le then suivant
        });

    })
    .then(function(doc) {
      if (!doc) {
       // rejette la promesse si doc est null ou undefined
       return Promise.reject(new Error('Document is null for file: ' + file));
      }
